...
void Constraint::satisfyConstraint(int constraintTimes)
{
  ...
  if (p1->isMovable() && p2->isMovable())
  {
    ...
    Vec3 correctionVectorHalf = correctionVector * (constraintTimes > 14 ? 0.5 : doubleMove[constraintTimes - 1]);
    p1->offsetPos(correctionVectorHalf);
    p2->offsetPos(-correctionVectorHalf);
  }
  else if (p1->isMovable() && !p2->isMovable())
  {
    Vec3 correctionVectorHalf = correctionVector * (constraintTimes > 14 ? 1 : singleMove[constraintTimes - 1]);
    p1->offsetPos(correctionVectorHalf);
  }
  else if (!p1->isMovable() && p2->isMovable())
  {
