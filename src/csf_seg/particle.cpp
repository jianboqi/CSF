...
void Particle::satisfyConstraintSelf(int constraintTimes)
{
  ...
  for (std::size_t i = 0; i < neighborsList.size(); i++)
  {
    ...
    if (p1->isMovable() && p2->isMovable())
    {
      ...
      Vec3 correctionVectorHalf = correctionVector * (constraintTimes > 14 ? 0.5 : doubleMove1[constraintTimes]);
      p1->offsetPos(correctionVectorHalf);
      p2->offsetPos(-correctionVectorHalf);
    }
    else if (p1->isMovable() && !p2->isMovable())
    {
      Vec3 correctionVectorHalf = correctionVector * (constraintTimes > 14 ? 1 : singleMove1[constraintTimes]);
      p1->offsetPos(correctionVectorHalf);
    }
    else if (!p1->isMovable() && p2->isMovable())
    {
