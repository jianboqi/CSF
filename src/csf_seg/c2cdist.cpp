...
void c2cdist::calCloud2CloudDist(Cloth& cloth, csf::PointCloud& pc, std::vector<int>& groundIndexes,
...
{
  groundIndexes.resize(0);
  upperGroundIndexes.resize(0);
  lowerGroundIndexes.resize(0);
  ...
  for (std::size_t i = 0; i < pc.size(); i++)
  {
    ...
    else if (height_var > 0.0)
    {
      upperGroundIndexes.push_back(i);
    }
    else
    {
      lowerGroundIndexes.push_back(i);
    }
  }
}
