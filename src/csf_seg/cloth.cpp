...
Cloth::Cloth(const Vec3& _origin_pos, int _num_particles_width, int _num_particles_height, double _step_x,
             double _step_y, double _smoothThreshold, double _heightThreshold, int rigidness, double time_step)
  ...
  , num_particles_height(_num_particles_height)
{
  ...
  for (int i = 0; i < num_particles_width; i++)
  {
    ...
    {
      Vec3 pos(origin_pos.f[0] + i * step_x, origin_pos.f[1], origin_pos.f[2] + j * step_y);
...
void Cloth::saveToFile(std::string path)
{
  ...
  for (std::size_t i = 0; i < particles.size(); i++)
  {
    f1 << std::fixed << std::setprecision(8) << particles[i].getPos().f[0] << "	" << particles[i].getPos().f[2] << "	"
