...
For singleMove1, which means one of the two particles is unmovable, then we move
the other one only: if constraintTimes = 0: singleMove1 = 0 if constraintTimes =
1: singleMove1 = 0.3, i.e., each time we move 0.3 (scale factor of the total
distance) for a particle towards the other one if constraintTimes = 2:
singleMove1 = (1-0.3)*0.3+0.3 = 0.51 if constraintTimes = 3: singleMove1 =
(1-0.51)*0.3+0.51 = 0.657
...
...
private:
  ...
  // double mass;             // the mass of the particle (is always 1 in this
  // example)
  Vec3 acceleration; // a vector representing the current acceleration of the
                     // particle
  Vec3 accumulated_normal; // an accumulated normal (i.e. non normalized), used
...
public:
  Particle(Vec3 pos /*, double time_step*/)
      : movable(true),
        // mass(1),
        acceleration(Vec3(0, 0, 0)), accumulated_normal(Vec3(0, 0, 0)),
        // time_step2(time_step), already used into the call of
        // Cloth::addForce(), at initialization
        pos(pos), old_pos(pos) {
    isVisited = false;
    neibor_count = 0;
    pos_x = 0;
  ...
  Particle()
      : movable(true),
        ...
        accumulated_normal(Vec3(0, 0, 0)), pos(Vec3(0, 0, 0)),
        old_pos(Vec3(0, 0, 0)) {
    isVisited = false;
    neibor_count = 0;
    pos_x = 0;
  ...
  // TODO: acceleration is a constant and should be either initialized by a
  // const ref in the constructor
  //  or by addind a ref to the Cloth object and using it to get the
  //  acceleration.
  void addForce(Vec3 f) {
    acceleration =
        f; // mass was here for correctness. it's always 1 in this example;
  }
